    ...
    public class KioskService : IKioskService
    {
        ...
        public KioskScreenDataDto GetScreenDataForExpense(int expenseId)
        {
            ...
            result.Users = DataContext.Users.Select(u => new KioskUserInfoDto()
            {
                ...
                TotalQuantity = u.ExpenseItems.Where(e => e.Expense.Id == expense.Id).Sum(e => (decimal?)e.Amount) ?? (decimal)0,
                LastExpenseRowCreated = u.ExpenseItems.Where(e => e.Expense.Id == expense.Id).Max(e => (DateTime?)e.CreatedDate)
            }).ToArray();
        ...
        public KioskExpenseDto[] GetExpensesWithKioskEnabled()
        {
            var expenses = Expenses.FetchAllWithKioskEnabled();
            return expenses.Select(e => new KioskExpenseDto()
            {
                Id = e.Id,
                IconId = e.IconId,
                Name = e.Name
        ...
        {
            if (!expense.IsKioskModeAllowed)
                throw new InvalidOperationException("This expense is not allowed for Kiosk access.");
        }
    }
}
